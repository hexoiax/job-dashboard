    ...
    <div style={{background:"#F4EFE8",minHeight:"100vh",fontFamily:"'Jost',sans-serif",color:"#28200F"}}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* SCROLL ROW — hidden scrollbar, navigated by buttons */
        .srow-wrap { position:relative; }
        .srow { display:flex; gap:20px; overflow-x:auto; padding-bottom:4px; scrollbar-width:none; scroll-behavior:smooth; }
        .srow::-webkit-scrollbar { display:none; }
        .srow > * { flex-shrink:0; width:320px; }
        .nav-btn {
          position:absolute; top:50%; transform:translateY(-50%);
          width:44px; height:44px; border-radius:50%;
          background:white; border:1.5px solid var(--border);
          box-shadow:0 4px 16px rgba(40,32,15,0.12);
          cursor:pointer; font-size:18px; display:flex; align-items:center; justify-content:center;
          transition:all 0.15s; z-index:10; color:var(--ink);
        }
        ...
        .nav-btn-inline {
          ...
          background:white; border:1.5px solid var(--border);
          box-shadow:0 2px 8px rgba(40,32,15,0.10);
          cursor:pointer; font-size:14px; display:flex; align-items:center; justify-content:center;
          transition:all 0.15s; color:var(--ink2); flex-shrink:0;
        }
        .nav-btn-inline:hover { background:var(--ink); color:var(--bg); border-color:var(--ink); }
        /* PANEL */
        ...
        <div style={{maxWidth:1440,margin:"0 auto",padding:"20px 32px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <div style={{flexShrink:0,paddingRight:20,borderRight:"1.5px solid var(--border)"}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:700,letterSpacing:"-0.01em",color:"var(--ink)",lineHeight:1}}>
                Job<span style={{color:"var(--acc)"}}>Radar</span>
              </div>
              <div style={{fontFamily:"Inconsolata,monospace",fontSize:12,color:"var(--ink3)",letterSpacing:"0.16em",textTransform:"uppercase",marginTop:3}}>
            ...
            <div style={{display:"flex",gap:24,flexShrink:0,paddingLeft:20,borderLeft:"1.5px solid var(--border)"}}>
              <div className="stat">
                ...
                <span style={{fontFamily:"Inconsolata,monospace",fontSize:12,color:"var(--ink3)",textTransform:"uppercase",letterSpacing:"0.10em"}}>Active</span>
              </div>
              <div className="stat">
                <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:700,color:"var(--green)"}}>{todayN}</span>
                <span style={{fontFamily:"Inconsolata,monospace",fontSize:12,color:"var(--ink3)",textTransform:"uppercase",letterSpacing:"0.10em"}}>Hôm nay</span>
              </div>
              <div className="stat">
                <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:700,color:"var(--acc)"}}>{verN}</span>
                <span style={{fontFamily:"Inconsolata,monospace",fontSize:12,color:"var(--ink3)",textTransform:"uppercase",letterSpacing:"0.10em"}}>Verified</span>
              </div>
              <div className="stat">
                <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:700,color:"var(--ink)"}}>{emailN}</span>
                <span style={{fontFamily:"Inconsolata,monospace",fontSize:12,color:"var(--ink3)",textTransform:"uppercase",letterSpacing:"0.10em"}}>Có Email</span>
              </div>
            </div>
          </div>
...
function FilterBlock({ label, children }) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <span style={{fontFamily:"Inconsolata,monospace",fontSize:13,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.14em",color:"var(--ink3)"}}>
        {label}
      </span>
  ...
  return (
    <div style={{marginBottom:24}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
        ...
        <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:700,color:"var(--ink)"}}>{title}</span>
      </div>
      {sub && <p style={{fontFamily:"Inconsolata,monospace",fontSize:13,color:"var(--ink3)",letterSpacing:"0.10em",textTransform:"uppercase",marginLeft:36}}>{sub}</p>}
    </div>
  );
}
...
function Shelf({ icon, title, sub, jobs, onSel }) {
  ...
  const SCROLL = 660;
  const scroll = (dir) => {
    if (rowRef.current) rowRef.current.scrollBy({ left: dir * SCROLL, behavior:"smooth" });
  };
  return (
      ...
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:20}}>
        ...
        {jobs.length > 3 && (
          <div style={{display:"flex",gap:8,marginBottom:6,flexShrink:0}}>
            <button className="nav-btn-inline" onClick={()=>scroll(-1)} title="Previous">◀</button>
            <button className="nav-btn-inline" onClick={()=>scroll(1)}  title="Next">▶</button>
          </div>
        )}
      </div>
      <div ref={rowRef} className="srow">
        {jobs.map((j,i)=>(
      ...
      <div style={{padding:"18px 20px 0"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:14,fontWeight:700,color:"var(--acc)",textTransform:"uppercase",letterSpacing:"0.04em"}}>
              {job["Tên Công Ty"]}
            </span>
            {job.isVerified && (
        ...
        {/* Title */}
        <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:23,fontWeight:700,lineHeight:1.2,color:"var(--ink)",marginBottom:14}}>
          {job["Vị Trí"]}
        </h3>

        {/* Salary */}
        <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:8}}>
          <span style={{fontFamily:"Inconsolata,monospace",fontSize:26,fontWeight:600,color:"var(--acc)"}}>
            {salLabel}
          </span>
        ...
        {job["Kỹ Năng"] && (
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:16}}>
            {job["Kỹ Năng"].split(",").slice(0,3).map(s=>s.trim()).filter(Boolean).map(s=>(
              <span key={s} style={{fontSize:12,padding:"4px 11px",border:"1px solid var(--border)",color:"var(--ink3)",borderRadius:20,fontWeight:500}}>
                {s}
              </span>
            ))}
          </div>
    ...
    <div className="overlay" onClick={onClose}>
      <div className="panel" style={{width:"min(92vw,960px)"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",height:"100%"}}>
          ...
          {/* RIGHT: CONTENT */}
          <div style={{flex:1,overflowY:"auto",background:"var(--bg)",padding:"36px 40px 60px"}}>

            {/* Close */}
            <button onClick={onClose} style={{
              marginBottom:32,padding:"10px 24px",fontSize:14,fontWeight:600,
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:20}}>
              ...
              {job.isVerified && (
                <span style={{display:"inline-flex",alignItems:"center",padding:"4px 12px",fontSize:12,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",borderRadius:3,fontFamily:"Inconsolata,monospace",background:"#E8F3FC",color:"#1A5A8A",border:"1px solid #9ECEF5"}}>
                  ✓ Verified Recruiter
                </span>
              )}
            </div>
            ...
            {/* Title */}
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:44,fontWeight:700,lineHeight:1.05,color:"var(--ink)",marginBottom:8}}>
              {job["Vị Trí"]}
            </h2>
            <p style={{fontSize:17,fontWeight:600,color:"var(--acc)",textTransform:"uppercase",letterSpacing:"0.10em",marginBottom:8}}>
              @ {job["Tên Công Ty"]}
            </p>

            {/* Posted date — prominent */}
            {job["Ngày đăng bài"] && (
              <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:20,padding:"5px 14px",marginBottom:28}}>
                <span style={{fontSize:13}}>📅</span>
                <span style={{fontFamily:"Inconsolata,monospace",fontSize:13,fontWeight:600,color:"var(--ink2)"}}>
                  Đăng ngày: {job["Ngày đăng bài"].split(" ")[1] || job["Ngày đăng bài"]}
                </span>
                {job.daysOld < 99 && (
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:28}}>
              {[
                ...
                <div key={l} style={{background:"white",border:"1.5px solid var(--border)",borderRadius:6,padding:"16px 18px"}}>
                  <div style={{fontFamily:"Inconsolata,monospace",fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.16em",color:a,opacity:0.9,marginBottom:6}}>{l}</div>
            ...
            {job["Nội Dung Gốc"] && (
              <div style={{marginBottom:28}}>
                <div style={{fontFamily:"Inconsolata,monospace",fontSize:13,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.16em",color:"var(--ink3)",marginBottom:14}}>Mô tả công việc</div>
                <p style={{fontSize:16,lineHeight:1.9,color:"var(--ink2)",whiteSpace:"pre-line"}}>{job["Nội Dung Gốc"]}</p>
              </div>
            )}

            {/* Benefits */}
            {job["Phúc Lợi"] && (
              <div style={{background:"#FFF8F0",border:"1.5px solid #E8C9A0",borderRadius:6,padding:22,marginBottom:32}}>
                <div style={{fontFamily:"Inconsolata,monospace",fontSize:13,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.16em",color:"var(--acc)",marginBottom:12}}>Phúc lợi & Quyền lợi</div>
            ...
            <div style={{marginTop:18,display:"flex",flexDirection:"column",gap:7}}>
              {job["Email"]&&job["Email"]!=="Không rõ" && (
                <p style={{fontFamily:"Inconsolata,monospace",fontSize:13,color:"var(--ink3)"}}>📧 {job["Email"]}</p>
              )}
              {job["SĐT"]&&job["SĐT"]!=="Không rõ" && (
                <p style={{fontFamily:"Inconsolata,monospace",fontSize:13,color:"var(--ink3)"}}>📞 {job["SĐT"]}</p>
              )}
            </div>
          </div>
        </div>
